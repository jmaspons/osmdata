...
trim_to_poly_pts <- function (dat, bb_poly, exclude = TRUE) {
    ...
    if (inherits (dat$osm_points, "sf")) {
        ...
        if (exclude) {
            ...
            g <- sf::st_transform (dat$osm_points, crs = osmdata:::.sph_merc ())
            bb_poly <- sf::st_transform (bb_poly, crs = osmdata:::.sph_merc ())
...
get_trim_indx <- function (g, bb, exclude) {
    ...
    if (exclude) {
        # st_contains_properly assumes planar coordinates. No s2 method?
        g <- sf::st_transform (g, crs = osmdata:::.sph_merc ())
        bb <- sf::st_transform (bb, crs = osmdata:::.sph_merc ())
...
verts_in_bpoly <- function (dat, bb_poly, exclude) {
    ...
    if (exclude) {
        # st_contains_properly assumes planar coordinates. No s2 method?
        g <- sf::st_transform (vert_to_sf (dat), crs = osmdata:::.sph_merc ())
        bb_poly <- sf::st_transform (bb_poly, crs = osmdata:::.sph_merc ())
